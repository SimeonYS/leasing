...
class LeasingPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `leasing`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM leasing WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO leasing (date, title, link, content)"
